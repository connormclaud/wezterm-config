...
- Hook events: `SessionStart` (startup|resume) → idle, `UserPromptSubmit` → running, `PreToolUse` → running (except `AskUserQuestion`/`ExitPlanMode` → asking), `PostToolUse` (`AskUserQuestion`/`ExitPlanMode`) → running, `PermissionRequest` → asking, `Notification` (permission_prompt, elicitation_dialog) → asking (backup), `SubagentStart` → running, `SubagentStop` → running, `Stop` → idle, `SessionEnd` → clear (no argument); configured in `~/.claude/settings.json`
